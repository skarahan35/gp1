...
export class CustomerGroupComponent {
  ...
  constructor(private http: HttpClient, private toastr:ToastrService) {
    this.dataSource = new CustomStore({
      key: 'id',
      load: () => this.sendRequest('https://localhost:44369/200404'),
      // insert: (values) => this.sendRequest('https://localhost:44369/200401', 'POST', values),
      // update: (key, values) => this.sendRequest(`https://localhost:44369/200402/${key}`, 'PUT', values),
      // remove: (key) => this.sendRequest(`https://localhost:44369/200403/${key}`, 'DELETE'),
    });
  ...
  onRowInserting(e: any) {
    e.cancel = true
    try {
      this.http.post('https://localhost:44369/200401', e.data).subscribe(
        (res: any) => {
  ...
  onRowUpdating(e:any){
    e.cancel = true
    try {
      this.http.put('https://localhost:44369/200402/' + e.key, e.newData).subscribe((res:any) => {
  ...
  onRowRemoving(e:any) {
    e.cancel = true
    try {
      this.http.delete('https://localhost:44369/200403/' + e.key).subscribe((res:any) => {
        debugger
